...
class FitMonitor(Callback):
    def __init__(self, **opt):
        ...
        # max difference between acc and val_acc for saving model
        self.thresh = opt.get('thresh', 0.02)
        # minimal accuracy for model saving
        self.minacc = opt.get('minacc', 0.99)
    ...
    def on_train_begin(self, logs={}):
        ...
        print("Train begin:", t)
        print("Stop file: %s (create this file to stop training gracefully)" %
              self.stop_file)
        print("Pause file: %s (create this file to pause training and view graphs)" %
              self.pause_file)
        self.print_params()
        self.progress = 0
        self.max_acc = 0
    ...
    def on_train_end(self, logs={}):
        ...
        if self.verbose:
            ...
            print("max_acc = %f  epoch = %d" %
                  (self.max_acc, self.max_acc_epoch))
            print("max_val_acc = %f  epoch = %d" %
                  (self.max_val_acc, self.max_val_acc_epoch))
        if self.filename:
            if self.checkpoint:
                print("Best model saved in file:", self.filename)
                print("Checkpoint: epoch=%d, acc=%.6f, val_acc=%.6f" %
                      self.checkpoint)
            else:
        ...
        if acc > self.max_acc:
            ...
            if self.filename != None:
                if acc > self.best_acc and (val_acc == -1 or abs(val_acc - acc) <= self.thresh):
                    print("\nSaving model to %s: epoch=%d, acc=%f, val_acc=%f" % (
                        self.filename, epoch, acc, val_acc))
                    self.model.save(self.filename)
...
def find_best_epoch(h, thresh=0.02):
    ...
    print("best epoch = %d ; best acc = %.6f ; best val_acc = %.6f" %
          (max_e, h.history['acc'][max_e], h.history['val_acc'][max_e]))
    return max_e
