...
def plot_data(inputs, labels):
    ...
    class_2_x, class_2_y, class_3_x, class_3_y = [], [], [], []
    ...
    for i in range(len(x0)):
        if labels[i] == 0:
            ...
            class_2_x.append(x0[i])
            class_2_y.append(x1[i])
        elif labels[i] == 3:
            class_3_x.append(x0[i])
            class_3_y.append(x1[i])

    plt.plot(class_0_x, class_0_y, 'x', color='r', label='Bus')
    plt.plot(class_1_x, class_1_y, '.', color='b', label='Saab')
    plt.plot(class_2_x, class_2_y, '*', color='g', label='Van')
    plt.plot(class_3_x, class_3_y, '+', color='m', label='Opel')
    plt.xlabel('x0')
    plt.ylabel('x1')
    plt.legend()
...
def main():
    ...
    # 1.
    # bus=>0, saab=>1, van=>2, opel=>3
    dataset = np.loadtxt(pre_path+'vechile.txt')
    _, col = dataset.shape
    inputs = dataset[:, 0:col-2]
    labels = dataset[:, col-1]
    X_train, X_test, y_train, y_test = train_test_split(
        inputs, labels, test_size=0.3, train_size=0.7, random_state=0, shuffle=True)

    inputs = utililty.pca(inputs)
    ...
    xx, yy = np.meshgrid(np.linspace(x0.min(), x1.max(
    ), num=100), np.linspace(x1.min(), x1.max(), num=100))

    two_feature_clf = svm.SVC(C=best_c, gamma=best_sigam, kernel='rbf')
    two_feature_clf.fit(inputs, np.ravel(labels))
    z = two_feature_clf.predict(np.array([xx.ravel(), yy.ravel()]).T).reshape(
        xx.shape)
